...
def show_openpose(keypoints_limbs, frame, **kwargs):
    ...
    off_y = 0
    if 'scale' in kwargs:
        scalex = kwargs['scale']['hscale']
        scaley = kwargs['scale']['vscale']
        off_x = kwargs['scale']['off_x']
        off_y = kwargs['scale']['off_y']
    frame = np.uint8(frame.copy())
    if len(keypoints_limbs) == 3:
        detected_keypoints = keypoints_limbs[0]
        #print(keypoints_limbs)
        personwiseKeypoints = keypoints_limbs[1]
        keypoints_list = keypoints_limbs[2]

        for i in range(nPoints):
            for j in range(len(detected_keypoints[i])):
                circle_center = detected_keypoints[i][j][0:2]
                cc = (int(circle_center[0]*scalex+off_x), int(circle_center[1]*scaley+off_y))
                cv2.circle(frame, cc, 5, colors[i], -1, cv2.LINE_AA)
        for i in range(17):
            for n in range(len(personwiseKeypoints)):
                ...
                cv2.line(frame, (int(B[0]*scalex+off_x), int(A[0]*scaley+off_y)), (int(B[1]*scalex+off_x), int(A[1]*scaley+off_y)), colors[i], 3, cv2.LINE_AA)
    return frame
